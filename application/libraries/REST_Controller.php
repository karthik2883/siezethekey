...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    {
		$string =  serialize($this->_args);
		$msg = $this->_filterBadWords($string);		
		if($msg!==true){
			 $response = array();
			 if(strpos(strtolower($_SERVER['PHP_SELF']),"/userpostcomment")===false){
             	$response['error'] = array('message' => $msg,'responseCode' => '111');
			 }else{
			 	$response['error'] = array('message' => 'Your comment has been sent','responseCode' => '111');			 
			 }				
			 $this->resPrint =false;
	...
	// Skip keys test for $config['auth_override_class_method']['class'['method'] = 'none'
        if (config_item('rest_enable_keys') and $this->auth_override !== true) {
            $x_api_key = $this->input->request_headers();
            if (isset($x_api_key['X-Api-Key'])) {
                $x_api_key = $x_api_key['X-Api-Key'];
            }else{
                $x_api_key = NULL;
            }
            $this->_allow = $this->_detect_api_key($x_api_key);
        }
        ...
        if($this->config->item('rest_auth_required') && in_array( strtolower($this->uri->segment(2)), $this->config->item('rest_auth_required'))){
            ...
            $token_name = 'HTTP_'.strtoupper(str_replace('-', '_', 'X-Auth-Token'));
            $authToken = $this->input->server($token_name);
			
            $userAuthData =  !empty($authToken) ? $this->user_auth_model->isValidToken($authToken) : '';
            ...
            if(!empty($userAuthData)){
                ...
                $response['message'] = $this->lang->line('INVALID_AUTH_TOKEN');
     ...
     * @return boolean
     */
    protected function _detect_api_key($x_api_key) {
         $api_key_variable = strtoupper(config_item('rest_key_value'));
         $x_api_key = strtoupper($x_api_key);
         if ($x_api_key <> $api_key_variable) {
            $message = ResponseMessages::getStatusCodeMessage(100);
            $undefined_api_key = array(
                'error_code'=>300,
                'message'=>$message,
                );
            $this->response($undefined_api_key, 500);
            exit;
         }else{
            return true;
         }
    ...
    /*public function generate_response($type, $message, $code, $data = array(),$header = array()){
            ...
            header('X-Response-Message: ' . $message);           
            ...
            $response = array();
            if($type == 'success'){
                    $response['success'] = array('message' => $message, 'data' => $data);	
            }else{
                    $response['error'] = array('message' => $message,'Code' => $code);
            }
    ...
    public function generate_response($response_Array = array()){
        $type = $response_Array['type'];
        $message = $response_Array['message'];
        if (is_numeric($message)) {
            $message = ResponseMessages::getStatusCodeMessage($response_Array['message']);
        }else{
            $message = $response_Array['message'];
        }
        switch ($type) {
            case SUCCESS:
                $type = $response_Array['type'];
                $code =  $response_Array['responseCode'];
                $serviceName = $response_Array['serviceName'];
            ...
            default:
                $type = $response_Array['type'];
                $message = ResponseMessages::getStatusCodeMessage(118);
                $code = 404;
                $serviceName = "Not Any Service";
                $data = NULL;  
            break;
        }
        $message = strip_tags(str_replace("\n",',',$message));
        header('X-Response-Message: ' . $message);          
        header('X-Response-Code: ' . $code);
        if (isset($response_Array['header'])) {
            $header = $response_Array['header'];
            if(count($header) > 0){
                foreach($header as $key => $value){
                    header($key. ' : ' . $value);
                }
